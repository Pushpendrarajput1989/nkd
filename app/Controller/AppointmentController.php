	...
	function practitioner_view($id=null){
		$joins = array(
			array(
				...
				'alias'=>'Condition',
				'type'=>'INNER',
				'conditions'=>'Condition.id = Appointment.condition'
			)
		);
		$booking = $this->Appointment->find('first',array('conditions'=>array('Appointment.id'=>$id),'joins'=>$joins,'fields'=>'Appointment.*,Condition.*'));
		$this->set('appointment',$booking);
	}
